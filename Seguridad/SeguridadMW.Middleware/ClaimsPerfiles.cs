...
namespace Autorizacion.Middleware
{
    public class ClaimsPerfiles
    {
        ...
        private async Task<IEnumerable<Abstracciones.Modelos.Perfil>> obtenerInformacionPerfiles(HttpContext httpContext)
        {
        ...
        {
            var usuario = await obtenerInformacionUsuarios(httpContext);
            if (usuario != null && !string.IsNullOrEmpty(usuario.Id.ToString()) && !string.IsNullOrEmpty(usuario.NombreUsuario.ToString()) && !string.IsNullOrEmpty
        ...
        private async Task<Abstracciones.Modelos.Usuario> obtenerInformacionUsuarios(HttpContext httpContext)
        {
            return await _autorizacionBW.ObtenerInformacionUsuario(new Abstracciones.Modelos.Usuario { NombreUsuario = httpContext.User.Claims.Where(c => c.Type == "usuario").FirstOrDefault().Value });
        }
    }
